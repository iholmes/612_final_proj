...
make.input <- function(alphamin, alphamax, tprobmin, tprobmax, nOffmin, nOffmax, step.size){
 ...
 off1mat <- cbind(rep(alphmat[,1], length(nOff)), rep(alphmat[,2], length(nOff)), 
                 rep(nOff, each=nrow(alphmat)))
 off2mat <- cbind(rep(off1mat[,1], length(nOff)), rep(off1mat[,2], length(nOff)), 
                rep(off1mat[,3], length(nOff)), rep(nOff, nrow(off1mat)))
...
Finally, I combine the within-host and transmission functions into one large model function, and apply that function to 
